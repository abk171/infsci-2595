---
title: "INFSCI 2595 Fall 2023 Homework: 05"
subtitle: "Assigned September 27, 2023; Due: October 4, 2023"
author: "Abhigyan Kishor"
date: "Submission time: October 4, 2023 at 11:00PM EST"
...
$$
z = \frac{x-\mu}{\sigma}\\
x_{0.25} = -0.67\times5 + 125 = 121.65\\
x_{0.75} = 0.67 \times5 + 125 = 128.35
$$

The interval between these constitutes 50% of the probability mass.
...
$$
p(\mu < 117) = z\left(\frac{117 - 125}{5}\right) \approx 0.0548
$$
...
We know, the posterior standard deviation can be expressed as:
...
When $\tau_0 \rightarrow \infty$,

$$
\frac{1}{\tau_N^2} \rightarrow \frac{N}{\sigma^2}\\
N \rightarrow \left(\frac{\sigma}{\tau_N}\right)^2
$$

When $\sigma = 15$ and $\tau_N = 3$,

$$
N = \left(\frac{15}{3}\right)^2 = 25
$$
...
$$
\log(p\left(\mu \mid \mathbf{x}, \sigma\right)) \propto \log(\prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \times \mathrm{normal}\left( \mu \mid \mu_0, \tau_0\right)) \\
\log(p\left(\mu \mid \mathbf{x}, \sigma\right)) \propto \sum_{n=1}^{N}log(\mathrm{normal}(x_n|\mu,\sigma) + \log(\mathrm{normal}(\mu | \mu_0,\tau_0))
$$

We can expand write the pdf for standard normal equation as:

$$
\mathrm{normal(x_n|\mu,\sigma)} = \frac{1}{\sqrt{2\pi\sigma^2}}\mathrm{exp}\left(-\frac{1}{2\sigma^2}(x_n - \mu)^2\right) 
$$

Taking log on both sides, we get,

$$
\log(\mathrm{normal(x_n|\mu,\sigma)}) = \log\left(\frac{1}{\sqrt{2\pi\sigma^2}}\mathrm{exp}\left(-\frac{1}{2\sigma^2}(x_n - \mu)^2\right) \right) \\
= -\frac{1}{2\sigma^2}(x_n - \mu)^2-\frac{1}{2}\log(2\pi\sigma^2)
$$

We can use this result for the second term. to simplify the first term, we will apply summation on both sides, as follows,

$$
\sum_{n=1}^N\log(\mathrm{normal(x_n|\mu,\sigma)}) = \sum_{n=1}^N\left(-\frac{1}{2\sigma^2}(x_n - \mu)^2-\frac{1}{2}\log(2\pi\sigma^2)\right)
$$

Putting it all together, we get

$$
\log(p(\mu |x,\sigma)) \propto \sum_{n=1}^N\left(-\frac{1}{2\sigma^2}(x_n - \mu)^2-\frac{1}{2}\log(2\pi\sigma^2)\right) -\frac{1}{2\tau_0^2}(\mu - \mu_0)^2-\frac{1}{2}\log(2\pi\tau_0^2)
$$
...
I will eliminate the constants in the equation and only consider variable $\mu$.

$$
\frac{d}{d\mu}\log(p(\mu |x,\sigma)) =  \frac{d}{d\mu}\sum_{n=1}^N\left(-\frac{1}{2\sigma^2}(x_n - \mu)^2\right) - \frac{d}{d\mu}\left(\frac{1}{2\tau_0^2}(\mu-\mu_0)^2\right)
$$

We can apply differentiation to all terms of the summation as follows,

$$
\frac{d}{d\mu}\log(p(\mu |x,\sigma)) =  -\frac{1}{2\sigma^2}\sum_{n=1}^N\frac{d}{d\mu}(x_n - \mu)^2 -\frac{1}{\tau_0^2}(\mu - \mu_0)\\
= \frac{1}{\sigma^2}\sum_{n=1}^N(x_n - \mu) -\frac{1}{\tau_0^2}(\mu - \mu_0)
$$

Using $\bar{x} = \frac{\sum x_n}{N}$ , we can write this equation in terms of $\bar{x}$ as follows,

$$
\frac{d}{d\mu}\log(p(\mu |x,\sigma)) = \frac{1}{\sigma^2}\left(N\bar{x} - \mu N\right) -\frac{1}{\tau_0^2}(\mu - \mu_0)
$$
...
$$
\frac{d}{d\mu}\log(p(\mu |x,\sigma)) = 0\\
\frac{N}{\sigma^2}\left(\bar{x} - \mu\right) -\frac{1}{\tau_0^2}(\mu - \mu_0)= 0\\
\mu\left(\frac{N}{\sigma^2}+\frac{1}{\tau_0^2}\right) = \frac{\mu_0}{\tau_0^2} - \frac{N\bar{x}}{\sigma^2}\\
\mu_{MAP} = \frac{\frac{\mu_0}{\tau_0^2} - \frac{N\bar{x}}{\sigma^2}}{\left(\frac{N}{\sigma^2}+\frac{1}{\tau_0^2}\right)}
$$
...
It is same as the posterior mean in the lecture. This is because in this case, the posterior is also a gaussian distribution, therefore it should have same properties.
...
$$ 
\log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto \log(\prod_{n=1}^{N} \left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) \times  \mathrm{Exp}\left( \sigma \mid \lambda \right))
$$

I will use results from problem 2 to simplify the first term as follows,

$$
\log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto\sum_{n=1}^N\left(-\frac{1}{2\sigma^2}(x_n - \mu)^2-\frac{1}{2}\log(2\pi\sigma^2)\right) + log(\lambda) - \sigma\lambda
$$
...
$$
\log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto\left(-\frac{Nv}{2\sigma^2}-\frac{1}{2}\log(2\pi\sigma^2)N\right) + log(\lambda) - \sigma\lambda\\
\propto \frac{Nv}{2\sigma^2} - N\log(\sigma) - \sigma\lambda
$$
...
{
  # your code here
  - sigma * lambda - (N*v)/(2 * sigma * sigma) - N * log(sigma)
}
```
...
tibble::tibble(
  ...
  mutate(s_unnorm = log_post_sigma_unnorm(sigma,N,v,0.75)) %>%
  ggplot(mapping = aes(x = sigma, y = s_unnorm)) +
  geom_line()
```

What do you think?

The shape is not like a parabola. It is unimodal.
...
Using the link function, we can calculate the inverse link function, $g^{-1}(\varphi)$ as follows,
...
$$
\frac{d}{d\varphi} e^\varphi = e^\varphi
$$

$e^x$ is positive throughout its domain, so we can ignore the mod.

Taking log on both sides, we get,

$$
\log\left(\frac{d}{d\varphi} e^\varphi\right) = \varphi
$$

Substituting into the above equation, we get,

$$
\log\left(p_{\varphi} \left(\varphi \mid \mathbf{x}, \mu\right) \right) \propto \frac{N}{2}\left(\frac{v}{e^{2\varphi}}-\frac{\log(2\pi e^{2\varphi})}{2}\right) + log(\lambda) - e^\varphi\lambda + \varphi
$$
...
tibble::tibble(
  ...
  mutate(s_unnorm = log_post_varphi_unnorm(varphi,N,v,0.75)) %>%
  ggplot(mapping = aes(x = varphi, y = s_unnorm)) +
  geom_line()
```

What do you think?

Yes, it looks more parabolic.
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = varphi, y = s_unnorm)) +
  geom_line() +
  geom_vline(color = 'red', xintercept = varphi_opt_a$par, size = 1.1) +
  geom_vline(color = 'blue', xintercept = varphi_opt_b$par, linetype = 'dotted', size = 1.1) 
```
