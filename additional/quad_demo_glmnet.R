### LASSO regression with GLMNET and the polynomial demo problem
...
### create the appropriate design matrix for the 8th degree model
### do NOT include the intercept because GLMNET does NOT want the 
### intercept column

### the conventional way which includes the intercept column
model.matrix( y ~ poly(x, degree = 8, raw = TRUE), train_noisy) %>% colnames()

model.matrix( y ~ poly(x, degree = 8, raw = TRUE), train_noisy) %>% head()

### the way that removes the intercept column
...
coef(lasso_mod, s = 1e-4)
coef(lasso_mod, s = 0.001)
coef(lasso_mod, s = 0.1)
coef(lasso_mod, s = 2)
coef(lasso_mod, s = 100)

coefplot::coefplot(lasso_mod, lambda = 0.001) + theme_bw() + theme(legend.position = 'none')
coefplot::coefplot(lasso_mod, lambda = 0.1) + theme_bw() + theme(legend.position = 'none')
coefplot::coefplot(lasso_mod, lambda = 2) + theme_bw() + theme(legend.position = 'none')
coefplot::coefplot(lasso_mod, lambda = 100) + theme_bw() + theme(legend.position = 'none')
...
### refine the grid of lambda values
...
### we did not discuss standardization in this script, but...
### by default glmnet() standardizes the inputs before fitting!
### it does NOT standardize the response by default
### can check the documentation to be sure
# ?glmnet::glmnet