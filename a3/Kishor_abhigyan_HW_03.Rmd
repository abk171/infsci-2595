---
title: "INFSCI 2595 Fall 2023 Homework: 03"
subtitle: "Assigned September 13, 2023; Due: September 20, 2023"
author: "Abhigyan Kishor"
date: "Submission time: September 20, 2023 at 11:00PM EST"
...
```{r, solution_01b}
### your code here 
model_pred_df %>% summarise(mean(obs_class == 'event'))
```
...
model_pred_df <- 
  model_pred_df %>% mutate(y = ifelse(obs_class == 'event', 1, 0))
```
...
Yes, it matches the predictions match the model. This is because where the prediction is low, there is more clusters of non-event data and vice-versa.
...
model_class_0.5 %>% summarise(mean(obs_class == pred_class))
```
...
I verified this by **calculation**. The goal was to write a single code to generate both sensitivity and specificity. I group dataset by `obs_class` to get all rows with same observed classes, i.e. event with event, non-event with non-event. Now when I take mean for `pred_class == obs_class`, it will take the proportion between correct predictions to total observations of that group. Thank you for pointing out that the `group_by` prediction was wrong :)

```{r, solution_02d}
### add more code chunks if you need to
stats_0.5 <- model_class_0.5 %>%
  group_by(obs_class) %>%
  summarise(value = mean(pred_class == obs_class))
stats_0.5
```
...
### your code here
model_class_0.7 %>%
  summarise(mean(pred_class == obs_class))
```
...
```{r, solution_03d}
### add more code chunks if you need to
## count here .. check tutorial for groupby and summarise
stats_0.3 <- model_class_0.3 %>%
  group_by(obs_class) %>%
  summarise(value = mean(pred_class == obs_class))
stats_0.3 
...
## I calculate the fpr from the specificity.
rocv <- bind_rows(stats_0.3, stats_0.5, stats_0.7) %>%
  mutate(value = ifelse(obs_class == 'event', value, 1 - value))

## Then I create two columms which contain the senstivity and fpr of each threshold.
rocv_data = data.frame(
  stv = rocv %>% filter(obs_class == 'event') %>% pull(value),
  fpr = rocv %>% filter(obs_class == 'non_event') %>% pull(value)
)
...
rocv_data %>%
  ggplot() +
  geom_point(mapping = aes(x = fpr, y = stv)) +
...
Yes, they are nearly balanced, leaning slightly towards non_event.
...
ctrl_acc <- caret::trainControl(method = 'cv', number = 5)
...
### model 1 linear additive features all inputs
set.seed(2021)
mod_1_acc <- train(y ~ x1 + x2 + x3 + x4, method = 'glm', trControl = ctrl_acc, data = df)
```
...
mod_2_acc <- train(y ~ I(x1)^2 + I(x2)^2 + I(x3)^2 + I(x4)^2, 
                   method = 'glm', trControl = ctrl_acc, data = df)
```
...
```{r, solution_04e_02}
### add your code here
summary(metric = metric_acc, acc_results)
```
...
```{r, solution_04e_03}
### add your code here
dotplot(metric = metric_acc, acc_results)
```

Which model is the best?

Model 6, i.e, quadratic between 2 and 3.
...
confusionMatrix(mod_6_acc)
confusionMatrix(mod_7_acc)
...
mod_1_roc <- train(y ~ x1 + x2 + x3 + x4,
                   method = 'glm', trControl = ctrl_roc, data = df)
```
...
```{r, solution_05c_02}
### add your code here
summary(metric = metric_roc, roc_results)
```
...
```{r, solution_05c_03}
### add your code hered
dotplot(metric = metric_roc, roc_results)
```

Which model is the best?

mod_3 i.e. quadratic between x1 and x2.
...
**Pipe the prediction data object for the `mod_1_roccaret` object to the `roc_curve()`. The `obs` column is the observed outcome and the `event` column is the model predicted event probability. Display the result to the screen to confirm the `roc_curve()` function worked. If it did the first few rows should correspond to very low threshold values.**
...
Sensitivity is $\frac{TP}{TP + FN}$ . At such a low threshold, we will classify everything as event. When we classify everything as event, our TP will be very high compared to FN. So we have value of 1.
...
It is hard to say which is best model without calculating the area under the curve. However, model 3, 6, 7 are good candidates for best models. Yes, this figure is consistent with the previous findings. Model 4 is closest to 45 degree line so it must be the most ineffective.
...
(pred_test_1 <- predict(type = 'prob', mod_1_roc, newdata = df_test))
```
...
No it is not very good. This is because where the model predicts low probability of event, we still get nearly half events (i.e. 0-0.3). However, the model gets good from the 0.4-1 range.
...
my_calcurve_1 <- test_df_1_b %>%
  ...
  mutate(mid_bin = seq(0.05, 0.95, by = 0.10))
```
...
Yes, it is same, however, because the calibration curve mapped event above and non_event below, so it looks flipped.
...
```{r, solution_07c}
### add more code chunks if you like
pred_test_3 <- predict(type = 'prob', mod_3_roc, newdata = df_test)
pred_test_8 <- predict(type = 'prob', mod_8_roc, newdata = df_test)
```
...
my_calcurve_8 <- test_df_8_b %>%
  group_by(pred_bin) %>%
  summarise(prop_event = mean(y == 'event')) %>%
  mutate(mid_bin = seq(0.35, 0.65, by = 0.10))
```
