---
title: "INFSCI 2595 Fall 2023 Homework: 07"
subtitle: "Assigned October 20, 2023; Due: October 27, 2023"
author: "Abhigyan Kishor"
date: "Submission time: October 27, 2023 at 11:00PM EST"
...
The small data matrix has 5 rows and 3 columns. The big data matrix has 30 rows and 3 columns.
...
There are 3 rows and 3 columns in both the covariance matrices.
...
No the answer does not change because $\beta_{k+1}$ does not depend on $\sigma$.
...
```{r}
qnorm(0.025, mean = postmeans_quad_small_noprior[3,1], sd = 1)
```

```{r}
qnorm(0.975, mean = postmeans_quad_small_noprior[3,1], sd = 1)
```

Yes, since the sign is same, for small data case the quadratic feature is statistically significant.

```{r}
qnorm(0.025, mean = postmeans_quad_big_noprior[3,1], sd = 1)
```

```{r}
qnorm(0.975, mean = postmeans_quad_big_noprior[3,1], sd = 1)
```

Yes, since the sign is same, for big data case the quadratic feature is statistically significant.
...
covmat_quad_small_inform <- solve(solve(B0_inform) + t(Xmat_quad_small) %*% Xmat_quad_small)
covmat_quad_small_inform
```

```{r, solution_for_2b_b, eval=TRUE}
### posterior covariance matrix INFORMATIVE prior and BIG data
covmat_quad_big_inform <- solve(solve(B0_inform) + t(Xmat_quad_big) %*% Xmat_quad_big)
...
covmat_quad_small_vague <- solve(solve(B0_vague) + t(Xmat_quad_small) %*% Xmat_quad_small)
covmat_quad_small_vague
```

```{r, solution_for_2c_b, eval=TRUE}
### posterior covariance matrix VAGUE prior and BIG data
covmat_quad_big_vague <- solve(solve(B0_vague) + t(Xmat_quad_big) %*% Xmat_quad_big)
...
In the case of the small data set, the results are very different. However, in the case of the bigger data set, the results are similar.
...
In the vague case, both data sets show similar results to no prior case.
...
laplace_quad <- my_laplace(c(-1,-2,-300,4), lm_logpost, info_quad)
```
...
set.seed(87123)
post_samples_quad <- generate_lm_post_samples(laplace_quad, 3, 2500)
```
...
### Uncertainty 
c(
  sort(post_samples_quad$beta_02)[0.025*2500],
  sort(post_samples_quad$beta_02)[0.975*2500]
)
## quantile function
```

```{r}
post_samples_quad %>% summarise(mean(beta_02 > 0))
```
...
post_trend_samples_quad <- make_post_lm_pred(Xmat_quad_big, post_samples_quad %>% select(-4))
...
### 
c(post_trend_samples_quad %>% nrow(), post_trend_samples_quad %>% ncol())
```

What do the rows and columns correspond to?

Each column corresponds to a certain y value obtained from x \* b.
...
We are averaging over all MAE values of that particular y.
...
absError_quad_mat <-  abs(post_trend_samples_quad - y_quad_big_col %>% as.vector())
### dimensions?
#post_trend_samples_quad %>% class()
#y_quad_big_col %>% class()
c(absError_quad_mat %>% nrow(),absError_quad_mat %>% ncol())
```
...
### data type?
MAE_quad %>% class() #numeric
### length?
length(MAE_quad) #2500
```
...
### your code here
mae_quad_post_01 - MAE_quad[1] #0
mae_quad_post_02 - MAE_quad[2] #~0
mae_quad_post_03 - MAE_quad[3] #0
```
...
Because our mode values are uncertain (due to sigma)
...
train_df %>%
  ...
  ggplot(mapping = aes(x = x, y = y)) +
  geom_point(mapping = aes(color = from))
```
...
make_spline_basis_mats <- function(J, train_data, test_data)
{
  ...
  test_basis <- splines::ns(x = test_data$x, df = J)
  
  interior_knots <- as.vector(attributes(train_basis)$knots)
  
  boundary_knots <- as.vector(attributes(train_basis)$Boundary.knots)
  
  train_matrix <- model.matrix(y ~ train_basis, data = train_data)
  
  test_matrix <- model.matrix(y ~ test_basis, data = test_data)
...
manage_spline_fit <- function(Xtrain, logpost_func, my_settings)
{
  ...
  print(dim(Xtrain))
  init_beta <- rnorm(Xtrain %>% ncol())
  
  init_varphi <- rexp(1, rate = my_settings$sigma_rate)
...
calc_mae_from_laplace <- function(mvn_result, Xtrain, Xtest, y_train, y_test, num_samples)
{
  ...
  post <- generate_lm_post_samples(mvn_result, Xtrain %>% ncol(), num_samples)
...
all_spline_mae_results %>%
  ...
  stat_summary(geom = 'line', fun = 'median', mapping = aes(color = dataset)) + 
  stat_summary(geom = 'point', fun = 'median', mapping = aes(color = dataset)) + 
...
The single train/test split are different to evedence-based assessment. The conclusions are different because the penalty in the metrics is different. In evidence based, we are considering the hessian matrix of the posterior mode while in the train/test split we care about absolute errors.
