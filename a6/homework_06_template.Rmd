---
title: "INFSCI 2595 Fall 2023 Homework: 06"
subtitle: "Assigned October 4, 2023; Due: October 11, 2023"
author: "Abhigyan Kishor"
date: "Submission time: October 11, 2023 at 11:00PM EST"
...
```{r, solution_01a}
### your code here
hw06_tb <- tibble::tibble(
  obs_id = hw06_df$obs_id,
  x = hw06_df$x
)
```

```{r}
hw06_tb %>% summarise(mean(x))
```

```{r}
hw06_tb %>% summarise(sd(x))
```

```{r}
hw06_tb %>% summarise(min(x))
```

```{r}
hw06_tb %>% summarise(max(x))
```
...
$$
\frac{d^2}{d\mu^2}\log(p(\mu |x,\sigma)) = \frac{d}{d\mu}\left(\frac{1}{\sigma^2}\left(N\bar{x} - \mu N\right) -\frac{1}{\tau_0^2}(\mu - \mu_0)\right)\\
...
We can use the second derivative test. Accordingly, we will plug our $\mu_{MAP}$ value into the second derivative.

$$
\left.\frac{d^2}{d\mu^2}\log(p(\mu |x,\sigma))\right|_{\mu = \mu_{MAP}} =  -\frac{N}{\sigma^2} - \frac{1}{\tau_0^2} < 0
$$

From the second derivative test, if $f'(x) = 0$ and $f''(x) < 0$, then the point is local maximum. Therefore we can be certain that $\mu_{MAP}$ corresponds to the value associated with the maximum log-posterior density.
...
For this case, we have $\hat{\theta} = [\mu]^T$. The laplace approximation is given as follows:

$$
p(\theta | \textbf{x}, \sigma) \approx \mathcal{N}\left(\hat{\theta},[\left.-H\right|_{\theta = \hat{\theta}}]^{-1}\right)
$$

The laplace approximation posterior mean is simply $\mu_{MAP}$ because we evaluate $g_{\theta} = 0$.

The laplace approximation posterior standard deviation can be calculated using the hessian matrix as:

$$
H = \left[\frac{\partial ^2}{\partial \mu} \log(p(\theta|\textbf{x},\sigma))\right]\\
[\left.-H\right|_{\theta = \hat{\theta}}]^{-1}= \left[-\left(-\frac{N}{\sigma^2} - \frac{1}{\tau^2}\right)\right] ^ {-1} = \sigma_N^2
$$

We can rewrite this in terms of precision as:
...
The result of a laplace approximation is a new normal distrbution. However, since we were certain of $\sigma$, our posterior is also a gaussian. So naturally, if you were to approximate a gaussian as a gaussian, you would get the same gaussian.
...
The laplace approximation assumes that standard deviation is not bounded. However, in reality, standard deviation must not be negative. Therefore we must transform it to a variable that is unbounded.
...
$$
p(\mu,\varphi | \textbf{x}) = \prod_{n=1}^N\{p(x_n|\mu, g^{-1}(\varphi))\}.\text{normal}(\mu|\mu_0,\tau_0).\text{Exp}(g^{-1}(\varphi)|\lambda).\left|\frac{d}{d\varphi}g^{-1}(\varphi)\right|
$$
...
$$
\frac{d}{d\varphi}\sigma = e^\varphi\\
\log\left(\frac{d}{d\varphi}\sigma\right) = \varphi
$$
...
Yes, identified optimal parameter values are same. Yes, Hessian matrices are same. The difference was in the counts i.e. the number of calls to each function. The log posterior surface has only 1 peak, i.e. uni-modal. Therefore, the optimisation for maximum would only end up in 1 point.
...
```{r}
cat("The mean values of the approximation are:\nmu \t= ", laplace_result$mode[1], "\nvarphi \t= ", laplace_result$mode[2], "\n")
cat("The standard deviations of each parameter are:\nsigma_mu \t= ",sqrt(laplace_result$var_matrix[1,1]), "\nsigma_varphi \t= ", sqrt(laplace_result$var_matrix[2,2]), "\n")
cat("The correlation coefficient is:\n rho = ", laplace_result$var_matrix[1,2]*sqrt(laplace_result$var_matrix[1,1])*sqrt(laplace_result$var_matrix[2,2]))
```
...
I will calculate the probability by using the `pnorm` function.

```{r}
pnorm(hw06_info$mu_0, mean = laplace_result$mode[1], sd = laplace_result$mode[2])
```
...
The posterior on $\sigma$ does not look like a gaussian.
