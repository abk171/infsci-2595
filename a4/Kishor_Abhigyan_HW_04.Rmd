---
title: "INFSCI 2595 Fall 2023 Homework: 04"
subtitle: "Assigned September 20, 2023; Due: September 27, 2023"
author: "Abhigyan Kishor"
date: "Submission time: September 27, 2023 at 11:00PM EST"
...
hit_data %>% summarise(H_avg = mean(hit == 1))
```
...
The MLE is simply the proportion of hits to total number of deliveries. It is same as calculating average of x.
...
$$
p(x_n|\mu) = \mu^{x_n}(1-x_n)\mu^{1-x_n}
$$

Taking log on both sides,

$$
\log(p(x_n|\mu)) = \log\left(\mu^{x_n}(1-x_n)\mu^{1-x_n}\right)
$$

Using product rule of logs, $\log(ab) = \log(a) + \log(b)$,

$$
\log(p(x_n|\mu)) = \log(\mu^{x_n})+\log(\mu^{1-x_n})
$$

Using exponent rule of logs, $\log(a^b) = b\log(a)$,

$$
\log(p(x_n|\mu)) = (x_n)\log(\mu)+(1-x_n)\log(\mu)
$$
...
The length of the result is equal to the length of the input `xobs`. The individual values are the same which is to be expected.
...
$$
p(\textbf{x}|\mu) = \prod_{n = 1}^N\left(\mu^{x_n}(1 - \mu)^{1-x_n}\right)
$$

Taking natural log on both sides,

$$
\log[p(\textbf{x}|\mu)] = \log\left(\prod_{n = 1}^N\left(\mu^{x_n}(1 - \mu)^{1-x_n}\right)\right)
$$

Using multiplication rule of logarithms, $\log(ab) = \log(a) + \log(b)$,

$$
\log[p(\textbf{x}|\mu)] = \sum_{n = 1}^N\left(\log\left(\mu^{x_n}(1 - \mu)^{1-x_n}\right)\right)
$$

We can simplify further using exponent rule of logarithms, $\log(a^b) = b\log(a)$,

$$
\log[p(\textbf{x}|\mu)] = \sum_{n = 1}^N x_n\log(\mu) + \sum_{n = 1}^N (1-x_n)\log(1-\mu)
$$
...
for (n in seq_along(mu_grid)) {
  log_lik_xa[n] <- log_bernoulli_pmf(x, mu_grid[n]) %>% sum()
}
log_lik_xa
```
...
xa_results %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) +
  geom_line() +
  geom_vline(xintercept = sum(x) / length(x))
```
...
First i will calculate the split of the dataset.
...
There are 14 1s and 20 0s.

According to the graph, as $\mu$ increases, the graph goes up until a threshold and then below again. It reaches peak at MLE value as expected. This behaviour is consistent with the formula because at low $\mu$ the contribution of hits is minimal while contribution of non-hits is significant. The imbalance in the legs can be explained because we have more non-hits than hits.
...
$$
\log \left( p \left( H \mid AB, \mu \right) \right) \propto H\log(\mu) + (N-AB)\log(1-\mu)
$$
...
tibble::tibble(
  ...
  mutate(log_lik = 14 * log(mu) + 20 * log(1 - mu)) %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) +
  geom_line(size = 1.1) + 
  geom_vline(xintercept = sum(x) / length(x))
...
It is the same. Because we ignore the binomial coefficient.
...
tibble::tibble(
  ...
  mutate(log_lik = dbinom(14, 34, mu, log = TRUE)) %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) +
  geom_line(size = 1.1) + 
  geom_vline(xintercept = sum(x) / length(x))
```
...
It looks same but infact it lower than the previous figure because `dbinom()` considers the binomial coefficient.
...
With uniform, it is 2 and with inform it is 40.
...
The two priors are very different. The uniform case suggests that all $\mu$ values have equal likelihood while the inform case suggests a particular range of more likely $\mu$ values. However, no value is ruled out in either case as shown in the following code block.

```{r}
dbeta(mu_grid, a_inform, b_inform)[0:10]
```

Even where it looks (from the graph) that there might be no likelihood, there is in fact some marginal likelihood of that $\mu$ value.
...
$$ 
\log \left( p \left( m \mid N, \mu \right) \right) \propto ?  
$$
...
$$ 
\log \left( p \left( \mu \mid a, b \right) \right) \propto ?  
$$
...
$$ 
\log(p(\mu | m, N) \propto m\log\mu + (N-m)\log(1 - \mu) + (a-1)\log\mu + (b-1)\log(1-\mu)
$$

$$
\log(p(\mu | m, N) \propto (m + a - 1)\log\mu + (N + b - m - 1)\log(1-\mu)
$$

Therefore, $a_{new} = m+a$ and $b_{new} = N + b - m$.
...
$$
\frac{d}{d\mu}\log(p(\mu | m, N)) = \frac{a_{new}-1}{\mu} - \frac{b_{new}-1}{1 - \mu}
$$
...
$$ 
\frac{d}{d\mu}\log(p(\mu | m, N)) = 0
$$

$$
\frac{a_{new}-1}{\mu} - \frac{b_{new}-1}{1 - \mu} = 0
$$

$$
\frac{(a_{new}-1)(1-\mu) - (b_{new}-1)\mu}{\mu(1 - \mu)} = 0
$$

$$
\frac{a_{new} - 1 +\mu(-b_{new} -a_{new}+ 2)}{\mu(1 - \mu)} = 0
$$
...
a_uniform_up <- 1 + 14
b_uniform_up <- 20 + 1
a_inform_up <- a_inform + 14
b_inform_up <- b_inform + 20
...
tibble::tibble(
  ...
  mean = c(a_uniform_up / (a_uniform_up + b_uniform_up), a_inform_up / (a_inform_up + b_inform_up)),
  mode = c(a_uniform_up / (1 + b_uniform_up), a_inform_up / (1 + b_inform_up)),
  q5 = c(qbeta(0.05, a_uniform_up, b_uniform_up), qbeta(0.05, a_inform_up, b_inform_up)),
  q95 = c(qbeta(0.95, a_uniform_up, b_uniform_up), qbeta(0.95, a_inform_up, b_inform_up))
)
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_uniform_up, b_uniform_up)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) + 
  geom_line() + 
  geom_point()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_inform_up, b_inform_up)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) + 
  geom_line() + 
  geom_point()
```
...
RED = beta distribution of updated posterior

BLUE = beta distribution of prior

BLACK = beta distribution of new data
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = mu, y = beta_pdf)) + 
  geom_line(color = 'red') + 
  geom_line(color = 'blue', mapping = aes(y = dbeta(mu, a_uniform, b_uniform))) + 
  geom_line(color = 'black', mapping = aes(y = dbeta(mu, 14, 20)))
```

As we can see, for the uniform case, the updated beta distribution is very sensitive to the new data values.
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = mu, y = beta_pdf)) + 
  geom_line(color = 'red') + 
  geom_line(color = 'blue', mapping = aes(y = dbeta(mu, a_inform, b_inform))) + 
  geom_line(color = 'black', mapping = aes(y = dbeta(mu, 14, 20)))
```

As we can see, in the informative case, while there is change in the distribution, it is not as sensitive to new data.
...
tibble::tibble(
  prior = c('uniform', 'informative'),
  mean = c(a_u_up / (a_u_up + b_u_up), a_i_up / (a_i_up + b_i_up)),
  mode = c(a_u_up / (1 + b_u_up), a_i_up / (1 + b_i_up)),
  q5 = c(qbeta(0.05, a_u_up, b_u_up), qbeta(0.05, a_i_up, b_i_up)),
  q95 = c(qbeta(0.95, a_u_up, b_u_up), qbeta(0.95, a_i_up, b_i_up))
)
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_u_up, b_u_up)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) + 
  geom_point() +
  geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_i_up, b_i_up)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) + 
  geom_point() +
  geom_line() 
```
...
RED = beta distribution of updated informative posterior

BLUE = beta distribution of updated uniform posterior
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = mu, y = beta_pdf)) + 
  geom_line(color = 'red') +
  geom_line(mapping = aes(y = dbeta(mu,season_hits, season_atbats - season_hits)), color = 'black') +
  geom_line(mapping = aes(y = dbeta(mu, a_u_up, b_u_up)), color = 'blue')
```

From the graph, as the data size increases, we see both posteriors almost agree on the $\mu$ value.
