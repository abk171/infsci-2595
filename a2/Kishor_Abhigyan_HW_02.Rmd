---
title: "INFSCI 2595 Fall 2023 Homework: 02"
subtitle: "Assigned September 6, 2023; Due: September 13, 2023"
author: "Abhigyan Kishor"
date: "Submission time: September 13, 2023 at 11:00PM EST"
...
The relationship seems non-linear because its decreases and then increases again.
...
rmse_1 = modelr::rmse(fit_lm_1, df_low_high) #7.717771
rmse_2 = modelr::rmse(fit_lm_2, df_low_high) #4.942079
rmse_3 = modelr::rmse(fit_lm_3, df_low_high) #4.581047
rmse_4 = modelr::rmse(fit_lm_4, df_low_high) #4.575364
rmse_5 = modelr::rmse(fit_lm_5, df_low_high) #4.575193
rmse_6 = modelr::rmse(fit_lm_6, df_low_high) #4.379204
rmse_7 = modelr::rmse(fit_lm_7, df_low_high) #4.373980
rmse_8 = modelr::rmse(fit_lm_8, df_low_high) #4.318560
rmse_9 = modelr::rmse(fit_lm_9, df_low_high) #4.279992

xv <- 0:8
y_rmse <- c(rmse_1, rmse_2, rmse_3, rmse_4, rmse_5, rmse_6, rmse_7, rmse_8, rmse_9)

ggplot(mapping = aes(x = xv, y = y_rmse)) +
  geom_line() +
  geom_point()
```
...
```{r, solution_01d}
### add as many code chunks as necessary
rsquare_1 = modelr::rsquare(fit_lm_1, df_low_high) #0.002595816
rsquare_2 = modelr::rsquare(fit_lm_2, df_low_high) #0.591015490
rsquare_3 = modelr::rsquare(fit_lm_3, df_low_high) #0.648587647
rsquare_4 = modelr::rsquare(fit_lm_4, df_low_high) #0.649458990
rsquare_5 = modelr::rsquare(fit_lm_5, df_low_high) #0.649485284
rsquare_6 = modelr::rsquare(fit_lm_6, df_low_high) #0.678872274
rsquare_7 = modelr::rsquare(fit_lm_7, df_low_high) #0.679638019
rsquare_8 = modelr::rsquare(fit_lm_8, df_low_high) #0.687704829
rsquare_9 = modelr::rsquare(fit_lm_9, df_low_high) #0.693257924

xv <- 0:8
y_rsquare <- c(rsquare_1, rsquare_2, rsquare_3, rsquare_4, rsquare_5, rsquare_6, rsquare_7, rsquare_8, rsquare_9)

ggplot(mapping = aes(x = xv, y = y_rsquare)) +
  geom_line() +
  geom_point()
```
...
According to the graphs above, it is clear that the 9th degree polynomial is the best model. This is because it has the lowest RMSE and highest $R^2$ value. Yes, both the performance metrics agree on the same model.
...
No, none of the models have similar coefficients. All three models of different levels of uncertainty.
...
input_viz <- tibble::tibble(
  x = seq(-2.34242, 2.64433, length.out =  201)
)
```
...
df_viz_1 %>%
  ggplot(mapping = aes(x = x)) +
  geom_point(mapping = aes(y = y), color = 'red', data = df_low_high, shape = 0) +
...
df_viz_all %>%
  ggplot(mapping = aes(x = x)) +
  geom_line(mapping = aes(y = pred_trend)) + 
  facet_wrap(~poly_degree, scale = 'free_y')
```
...
The best model (model 9) is very sensitive to the training set.
...
df_low_high %>%
  ggplot(mapping = aes(x = x, y = y)) +
    ...
    geom_point(data = df_holdout, shape = 0, color = 'red')
```
...
rmse_train_1 = modelr::rmse(fit_train_1, df_holdout) #14.649529
rmse_train_2 = modelr::rmse(fit_train_2, df_holdout) #7.877605
rmse_train_3 = modelr::rmse(fit_train_3, df_holdout) #7.994202
rmse_train_4 = modelr::rmse(fit_train_4, df_holdout) #9.623962
rmse_train_5 = modelr::rmse(fit_train_5, df_holdout) #66.528608
rmse_train_6 = modelr::rmse(fit_train_6, df_holdout) #110.592780
rmse_train_7 = modelr::rmse(fit_train_7, df_holdout) #547.302900
rmse_train_8 = modelr::rmse(fit_train_8, df_holdout) #852.592341
rmse_train_9 = modelr::rmse(fit_train_9, df_holdout) #3585.752884

xv <- 0:8
y_rmse <- c(rmse_train_1, rmse_train_2, rmse_train_3, rmse_train_4, rmse_train_5, rmse_train_6, rmse_train_7, rmse_train_8, rmse_train_9)
...
According to the hold-out evaluation, the best model is `rmse_train_2`. It is different from the model identified according to the training set performance. In fact, the hold-out set ranks the model (model 9) as worst performing.
...
The coefficients have different signs, because the model is very sensitive to the training set.
...
All coefficients have same sign, which indicates that the model is insensitive to training data.
...
Each model will be trained and tested 5 times. 5 folds means that we will split the data into 5 equal parts.
...
I used RMSE because we have used it earlier along with `Rsquared` to predict which model is best. It would be easier to compare model performance with the same metric.
...
### linear relationship (degree 1)
set.seed(2001)
mod_lowhigh_1 <- train(form = y ~ x, method = "lm", trControl = my_ctrl, data = df_low_high)
...
### quadratic relationship (degree 2)
set.seed(2001)
mod_lowhigh_2 <- train(form = y ~ x + I(x^2), method = "lm", trControl = my_ctrl, data = df_low_high)
...
### cubic relationship (degree 3)
set.seed(2001)
mod_lowhigh_3 <- train(form = y ~ x + I(x^2) + I(x^3), method = "lm", trControl = my_ctrl, data = df_low_high)
...
### degree 4
set.seed(2001)
mod_lowhigh_4 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4), method = "lm", trControl = my_ctrl, data = df_low_high)
...
### degree 5
set.seed(2001)
mod_lowhigh_5 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), method = "lm", trControl = my_ctrl, data = df_low_high)
...
### degree 6
set.seed(2001)
mod_lowhigh_6 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), method = "lm", trControl = my_ctrl, data = df_low_high)
...
### degree 7
set.seed(2001)
mod_lowhigh_7 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), method = "lm", trControl = my_ctrl, data = df_low_high)
...
### degree 8
set.seed(2001)
mod_lowhigh_8 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), method = "lm", trControl = my_ctrl, data = df_low_high)
...
### degree 9
set.seed(2001)
mod_lowhigh_9 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), method = "lm", trControl = my_ctrl, data = df_low_high)
...
summary(metric = my_metric, lowhigh_results)
```

```{r, solution_04e_b}
### dotplot for your performance metric
dotplot(metric = my_metric, lowhigh_results)
```

Which model is the best?

According to resampling, `mod_lowhigh3` is best.
...
```{r, solution_04f}
### your code here
coefplot::multiplot(mod_lowhigh_3, mod_lowhigh_1, mod_lowhigh_4)
```

What do you think?

Yes, There is similarity between `mod_lowhigh_4` and `mod_lowhigh_3`. Further, we can see that the level of uncertainty is similar in all 3 models.
...
It displays the trend a lot more clearly than the high noise data.
...
### linear relationship (degree 1)
set.seed(2001)
mod_lowlow_1 <- train(form = y ~ x, method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### quadratic relationship (degree 2)
set.seed(2001)
mod_lowlow_2 <- train(form = y ~ x + I(x^2), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### cubic relationship (degree 3)
set.seed(2001)
mod_lowlow_3 <- train(form = y ~ x + I(x^2) + I(x^3), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### degree 4
set.seed(2001)
mod_lowlow_4 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### degree 5
set.seed(2001)
mod_lowlow_5 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### degree 6
set.seed(2001)
mod_lowlow_6 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### degree 7
set.seed(2001)
mod_lowlow_7 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### degree 8
set.seed(2001)
mod_lowlow_8 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
### degree 9
set.seed(2001)
mod_lowlow_9 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_low_low)
...
According to the resampling on low noise data, `mod_lowlow_6` is the best model.
...
No the low noise resampling gave us model 6 while high noise resampling gave us model 3. However, both datasets agree on the worst model i.e. model 9.
...
There are more data points and the trend is more noisy on the left.
...
### linear relationship (degree 1)
set.seed(2001)
mod_highhigh_1 <- train(form = y ~ x, method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### quadratic relationship (degree 2)
set.seed(2001)
mod_highhigh_2 <- train(form = y ~ x + I(x^2), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### cubic relationship (degree 3)
set.seed(2001)
mod_highhigh_3 <- train(form = y ~ x + I(x^2) + I(x^3), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### degree 4
set.seed(2001)
mod_highhigh_4 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### degree 5
set.seed(2001)
mod_highhigh_5 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### degree 6
set.seed(2001)
mod_highhigh_6 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### degree 7
set.seed(2001)
mod_highhigh_7 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### degree 8
set.seed(2001)
mod_highhigh_8 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
### degree 9
set.seed(2001)
mod_highhigh_9 <- train(form = y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), method = "lm", trControl = my_ctrl, metric = my_metric, data = df_high_high)
...
The best model identified with `df_high_high` differs from the best model identified with `df_low_high`. However, both datasets agree that model 9 was the worst performing model.
...
The noise level and sample size work in opposite ways to affect the generalizing of a model. Model training is more robust when there is more data. However, when there is high noise in the dataset, then we will face error during the testing phase. s
