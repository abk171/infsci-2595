---
title: "INFSCSI 2595 Fall 2023 Homework: 01"
subtitle: "Assigned August 30, 2023; Due: September 6, 2023"
author: "Abhigyan Kishor"
date: "Submission time: September 6, 2023 at 11:00PM EST"
...
iris %>%
  ggplot(mapping = aes(x = Sepal.Length)) +
  geom_histogram(bins = 6, color = "black", fill = "white")
```
...
#  geom_histogram(bins = 6, color = "gold", fill = "navyblue")
```
...
iris %>%
  ...
  #geom_histogram(bins = 6, color = "navyblue", fill = "gold", linewidth = 1.55)
```
...
From the above boxplot, we can see that the color has an affect on the median value of the price. Different colors have different median and quartile prices.
...
When we use entries with `cut == "Ideal"` and `clarity == "IF"`, we notice that change in color reduces the price, which is opposite to what we see in problem **2b**.
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) +
  geom_point() +
  geom_smooth(method = lm, mapping = aes(color = drv), formula = y ~ x)
```
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) +
  geom_point() +
  geom_smooth(method = lm, mapping = aes(group = drv), formula = y ~ x)
```
...
Yes, the displacement variable `displ` is significant because its confidence interval does not intersect 0.
...
Yes, all the variables are significant as they do not intersect 0. There are 3 coefficients displayed; 2 for `drv` and 1 for `displ`.
...
mod3 <- lm(hwy ~ displ + drv, data = mpg_mod)
