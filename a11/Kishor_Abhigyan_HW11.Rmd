---
title: "INFSCI 2595 Fall 2023 Homework: 11"
subtitle: "Assigned November 17, 2023; Due: December 1, 2023"
author: "Abhigyan Kishor"
date: "Submission time: December 1, 2023 at 11:00PM EST"
...
df_small %>%
  ...
  ggplot(aes(value, y))+
  geom_jitter(aes(color = name), height = 0.02, width = 0.02) +
  geom_smooth(method = 'glm', method.args = list(family = 'binomial'), formula = y ~ x + I(x^2)) +
  facet_wrap(~ name)
```
...
No I don't see a single line that can divide the data points perfectly.
...
The model seems to have extremely high standard error on the estimates.
...
```{r}
df_small %>%
  ggplot(aes(x1, x2, color = as.factor(y), shape = as.factor(y))) +
  geom_point()
```
...
There is a central area which has high concentration of events, while the outer areas have non-events.
...
df %>%
  ...
  pivot_longer(c(x1, x2)) %>%
  ggplot(aes(value, y)) +
  geom_jitter(aes(color = name), width = 0.02, height = 0.02) +
  facet_wrap(~ name)
```
...
They have similar areas of concentration of the outcome.
...
```{r}
fit_glm <- glm(y ~ x3*(x1 + x2  + x1*x2 + I(x1^2) + I(x2^2)), data = df, family = 'binomial')
```
...
Mostly no. But some interactions are significant.
...
`pred_viz_glm_b` represents the event probability.
...
They look elliptical, and are dependent on `x3`.
...
enet_default <- caret::train(
  outcome ~ x3*(x1 + x2  + x1*x2 + I(x1^2) + I(x2^2)),
  data = df_caret,
  method = 'glmnet',
  trControl = my_ctrl,
  metric = my_metric,
...
Yes, the model has been simplified because the original design matrix has 18 coefficients.
...
```{r}
nnet_default <- caret::train(outcome ~ ., data = df_caret,
                             metric = my_metric,
                             trControl = my_ctrl,
                             method = 'nnet',
...
```{r}
nnet_tune <- caret::train(outcome ~ ., data = df_caret,
                          metric = my_metric,
                          trControl = my_ctrl,
                          method = 'nnet',
                          preProcess = c('center', 'scale'),
...
It is difficult to compare because the neural network does not show us the formula that it computes.
...
```{r}
rf_default <- caret::train(outcome ~ ., data = df_caret,
                           method = 'rf',
                           importance = TRUE,
                           metric = my_metric,
                           trControl = my_ctrl,
...
```{r}
xgb_default <- caret::train(outcome ~ ., data = df_caret,
                            metric = my_metric,
                            trControl = my_ctrl,
                            method = 'xgbTree',
                            verbosity = 0,
...
```{r}
xgb_tune <- caret::train(outcome ~ ., data = df_caret,
                            metric = my_metric,
                            trControl = my_ctrl,
                            method = 'xgbTree',
                            verbosity = 0,
